...
import { FreePlaybookComponent } from '../free-playbook/free-playbook.component';
...
@Component({
  selector: 'app-offense-public-dashboard',
  standalone: true,
  imports: [CommonModule, FormsModule, CarouselModule],
  templateUrl: './offense-public-dashboard.component.html',
  styleUrls: ['./offense-public-dashboard.component.css']
})
export class OffensePublicDashboardComponent implements OnInit {
  playbookId: any;
  limit = 0;
  page1 = 0;
  videos: any[] = [];
  ...
  dashBoardData() {
    ...
    this.playbookService.getOffensePublicPlabookDashboardData(this.playbookId, this.limit, environment.publicAcCessPlayBookId).subscribe(resp => {
  ...
  goToDeshtopFreeVersion() {
    window.open("https://fieldvisionupload.s3.amazonaws.com/StrongSide.exe", '_blank')
  }
}
