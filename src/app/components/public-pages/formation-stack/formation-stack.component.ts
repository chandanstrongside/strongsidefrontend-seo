...
import { TagModalComponent } from '../../../shared/components/modal/tag-modal/tag-modal.component';
import { NgSelectModule } from '@ng-select/ng-select';
@Component({
  selector: 'app-formation-stack',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, NgbModule, NgSelectModule],
  templateUrl: './formation-stack.component.html',
  styleUrls: ['./formation-stack.component.scss']
})
export class FormationStackComponent implements OnInit {
  @Input() playbookId: any;

  items: any[] = ['Offense'];
  page = new Page();
  rows = new Array<FormationStack>();
  ...
  //{id:8, value: 'excludeFreePlayBook', label:'Exclude Free PlayBook'}
  selected: any;
  ...
  modalData: any;
  @ViewChild("formationStackVideoViewContent", { static: true })
  formationStackVideoViewContent!: ElementRef<HTMLElement>;
  searchText: any;
  ...
  ngOnInit(): void {
    ...
    this.searchForm.get("names")?.valueChanges.subscribe((value: any) => {
      this.reloadTable();
    });
    this.searchForm.get("dateFilterType")?.valueChanges.subscribe((value: any) => {
      if (value != 0) {
    ...
    this.searchForm.get("lastUpdatedFrom")?.valueChanges.subscribe((value: any) => {
      this.reloadTable();
    });
    this.searchForm.get("lastUpdatedto")?.valueChanges.subscribe((value: any) => {
      this.reloadTable();
    });
    this.searchForm.controls["order"]?.valueChanges.subscribe((value: any) => {
      this.reloadTable();
    });
  ...
  reloadTable() {
    ...
    this.selectedRow = [];
    this.offensePublicService.publicFormationSetSearch(searchParam)
      .pipe(first())
      .subscribe((resp: any) => {
  ...
  tag(id: string, associate: string) {
  ...
  openVideoModal(data: any) {
    this.modalData = data;
    this.modalService.open(this.formationStackVideoViewContent, { centered: true, size: "xl" });
  };
